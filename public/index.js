...
const func = {
  ...
  showItemBeingUsed (item) {
    if (item === 'none') {
      ...
      document.querySelector('.show-item-used').src = `./Images/item-${item}.png`
    }
  },
  ...
  checkWin: function () {
    if (
      ...
      const activePlayer = Object.values(rules.activePlayer).filter(a => a).length
...
socket.on('pathHandler', ({ selectedPos, playerHorse }) => {
  ...
  if (
    playerHorse[1] == res.selectedPos ||
    playerHorse[2] == res.selectedPos
  ) {
    ...
    if (res.isShield) {
      ...
      if (
        res.selectedPos !=
        ((rules.finish[res.currentPlayer] - 1) % 56) + 1
      ) {
        let nextPosId =
        ...
        if (
          ...
          if (!kickSameHorse) {
            ...
            document.getElementById(
              `path-${func.toId(res.selectedPos)}`
            ).src = "";
            res.currentPos[res.currentPlayer][
              res.selectedHorse
            ] = nextPosRound;
...
const mainController = {
  diceHandler: function () {
    ...
    // if (res.currentDice === "wait" && res.currentPlayer == player.number) {
    //   res.currentDice = func.random();
  ...
  finishHandler: function (event) {
    func.playWithPermission(() => {
      ...
      res.selectedHorse = res.currentPos[res.currentPlayer][1] == res.selectedPos ? 1 : 2;
...
const itemController = {
  ...
  setTrap: function (event) {
    func.playWithPermission(() => {
      if (res.isTrap) {
        if (!res.itemActive.trap.includes(parseInt(event.target.id.split("-")[2]))) {
...
for (let i = 0; i < 3; i++) {
  document.getElementById(`itemage-${i}-area`).addEventListener("click", func.findSelectedItem);
}
...
// socket.emit('say-hi', { room: 'test-room', msg: 'hiiii' })
...
socket.on('playerLeave', ({ id }) => {
  console.log(`${id} has left this game`)
  rules.activePlayer[func.getPlayerNumber(id)] = false
})